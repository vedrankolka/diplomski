...
def create_sniffer(
    input_file, input_interface, output_mode, output_file, url_model=None, broker=None, topic=None
):
    assert (input_file is None) ^ (input_interface is None)

    NewFlowSession = generate_session_class(output_mode, output_file, url_model, broker, topic)
...
def main():
    ...
    parser.add_argument(
        "--broker",
        help="Kafka broker URL for streaming flow data"
    )

    parser.add_argument(
        "--topic",
        help="Name of topic on the Kafka broker"
    )
